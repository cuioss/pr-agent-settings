...
`max_model_tokens` is set to `128000`. Upstream defaults it to `32000`, and with
`large_patch_policy = "clip"` any diff above the limit is truncated *before the model reads it*.
A 100-file pull request was reviewed on a clipped diff under the default.
...
`128000` is a deliberate middle — roughly 4x the default, comfortably above the largest diff
observed here, and far below the 1M ceiling, because an unbounded window turns one pathological
pull request into a pathological bill.
