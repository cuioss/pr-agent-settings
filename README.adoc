...
reviewed as though complete; a dead setting that appears in every run's resolved-config dump.
...
The mitigation we do have is the ensemble: three reviewers with different mandates, where the
published benchmarks agree the tools disagree. API-Sheriff#118 is the concrete case — this
reviewer found a session-collision defect that both of the other two missed.
