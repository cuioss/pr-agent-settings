...
Verify a model id against the project before configuring it here. Leading with an unavailable
model is not merely cosmetic: it costs two failed calls and several seconds per run before the
fallback takes over, and before the gate below existed it cost one silent, review-less run per
pull request (see <<no-self-report>>).
