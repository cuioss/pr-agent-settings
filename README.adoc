...
`gemini-3.6-flash`, which now leads — newest generation, and it avoids the `429` backoff that
`gemini-2.5-pro` incurs. `gemini-2.5-pro` sits directly behind it, because a security-weighted
charter is the workload where pro-tier reasoning depth may still win; if review quality visibly
drops, swap the first two entries.
