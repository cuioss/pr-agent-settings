...
* *Its findings are strongest where the defect is local to the diff.* Both defects it has
recovered here — a session-identity collision and a sequence-number reuse — were visible within
the changed code once the model could see all of it. Do not expect it to reason about a caller
three files away that the pull request did not touch.
