...
model to confirm the account rather than examine the code — the same suppression pressure, arriving
as context instead of as instruction. If authored intent is ever routed here deliberately, it
belongs in the prompt as a *claim to be verified*: report where the implementation diverges from
it, and never treat conformance with it as evidence of correctness.
====
