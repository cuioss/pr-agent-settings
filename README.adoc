...
this region", and "wrong id". Rather than resolve it by hand, `model` leads with the strongest
candidate and `fallback_models` descends through the Pro tier to the Flash pair that is known to
serve. LiteLLM falls through on a `404`, so the configuration selects the best model the project
can actually serve, and the run log names which rung answered. Pin the winner and drop the dead
rungs once one run has spoken.
...
observed here (~22k tokens), and far below the 1M ceiling, because an unbounded window turns one
pathological pull request into a pathological bill.
